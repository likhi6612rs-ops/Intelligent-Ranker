...
function WeightSliders({ weights, onChange }: { weights: Weights; onChange: (w: Weights) => void }) {
  ...
  function handleSlider(key: keyof Weights, newVal: number) {
    ...
    if (currentOtherTotal === 0) {
      ...
      others.forEach((m, i) => {
        updated[m.key] = i === others.length - 1 ? remaining - split * (others.length - 1) : split;
      });
    } else {
      others.forEach(m => {
        updated[m.key] = Math.round((weights[m.key] / currentOtherTotal) * remaining);
      });
    }

    // Clamp all to [0, 100]
    WEIGHT_META.forEach(m => {
      updated[m.key] = Math.max(0, Math.min(100, updated[m.key]));
    });
    ...
    <div className="space-y-3 bg-white/[0.02] border border-white/8 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-3">
          {!isBalanced && (
            <span className="text-[10px] text-amber-400/70 font-mono">Total: {total}%</span>
          )}
          <button
            onClick={() => onChange(DEFAULT_WEIGHTS)}
            className="text-[10px] text-white/25 hover:text-white/50 font-mono flex items-center gap-1 transition-colors"
          >
      ...
      {WEIGHT_META.map(({ key, label, color, colorCls }) => (
        <div key={key} className="space-y-1.5">
          ...
          <div className="relative h-2 bg-white/5 rounded-full">
            <div
              className={`absolute inset-y-0 left-0 rounded-full transition-all ${colorCls} opacity-60`}
      ...
      <p className="text-[10px] text-white/20 font-mono pt-1">
        Adjusting one dimension auto-balances the others · Total must equal 100%
      </p>
    </div>
  );
}

// ─── Weighted display bar ─────────────────────────────────────────────────────
...
function CandidateCard({ ranked, weights }: { ranked: RankedCandidate; weights: Weights }) {
  ...
  const expLabel = isEntryLevel ? "Entry Level / No Prior Experience" : `${candidate.yearsOfExperiencePrecise.toFixed(1)} yrs`;
  ...
  return (
    <motion.div
      ...
      className={`border rounded-2xl overflow-hidden transition-all duration-200 ${
        rank === 1 ? "border-cyan-400/30 bg-cyan-400/[0.02]"
        : rank <= 3 ? "border-violet-400/20 bg-white/[0.02]"
    ...
    >
      ...
      >
        <span>Full breakdown & career details</span>
          ...
          >
            <div className="px-5 pb-5 space-y-5 border-t border-white/5 pt-4">
              <div className="space-y-3">
                <p className="text-[10px] font-mono text-white/25 uppercase tracking-widest mb-1">Score Breakdown</p>
        ...
        onError: async (err: unknown) => {
          ...
          if (resp?.response) {
            try {
              ...
              if (body.code === "INVALID_JD") {
                setJdError(body.error ?? "Invalid job description. Please provide a full description.");
                return;
              }
            } catch {
              // ignore parse error
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <header className="border-b border-white/5 px-6 py-4 flex items-center justify-between sticky top-0 z-20 bg-neutral-950/80 backdrop-blur">
        ...
        {results && (
          <button
            onClick={() => { setResults(null); setJdError(null); }}
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            <div>
              ...
              <p className="text-white/40 text-sm leading-relaxed">
                Paste a complete job description — the engine extracts required skills, experience thresholds, and role scope.
                Adjust the weights below to match your hiring priorities.
              </p>
            </div>
            ...
            <div className="space-y-2">
              <Textarea
                ...
                className={`min-h-[240px] bg-white/[0.03] text-white placeholder:text-white/20 resize-none text-sm leading-relaxed transition-colors ${
                  jdError ? "border-red-500/50 focus:border-red-500/70" : "border-white/10 focus:border-white/25"
                }`}
              />
        ...
        {isAnalyzing && (
          <div className="mt-8">
            <LoadingStages resumeCount={resumes.length} />
          </div>
        )}
